...
for i in range(1,merged_data.shape[1]-12):
    for j in range(merged_data.shape[1]-12,merged_data.shape[1]):
        df_rho.set_value(merged_data.columns[i],merged_data.columns[j], scipy.stats.pearsonr(np.array(merged_data.iloc[:,i][merged_data.iloc[:,j] != 'na']).astype(np.float),np.array(merged_data.iloc[:,j][merged_data.iloc[:,j] != 'na']).astype(np.float))[0])
        df_p.set_value(merged_data.columns[i], merged_data.columns[j],  scipy.stats.pearsonr(np.array(merged_data.iloc[:,i][merged_data.iloc[:,j] != 'na']).astype(np.float),np.array(merged_data.iloc[:,j][merged_data.iloc[:,j] != 'na']).astype(np.float))[1])
...
df_rho.to_csv('C:/Users/Anna/Documents/pearson_correlations_sick.csv')
df_p.to_csv('C:/Users/Anna/Documents/pearson_p_values_sick.csv')
print('done')
