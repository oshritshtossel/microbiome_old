...
from preprocess_and_distance_IBD import ibd
...
#
df, mapping_file = ibd(perform_distance=True,level =4)
otu_after_pca, _ = apply_pca(df, n_components=15)
#otu_after_pca = otu_after_pca.drop(otu_after_pca.columns[[6, 13, 15]], axis=1)
merged_data = otu_after_pca.join(mapping_file)
merged_data = merged_data.drop('#SampleID', axis=1)

X,y = merged_data.loc[:, merged_data.columns != 'CD_or_UC'], merged_data['CD_or_UC']
...
def train_and_eval(size, lr, weight_decay,dropout=0.35):
    ...
    #    print(label.data)
        ...
        #     if all(i < m_t-0.05 for i in test_accuracy[-50:]):
        #         break
