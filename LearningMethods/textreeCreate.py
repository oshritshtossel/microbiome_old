...
def create_tax_tree(series, remove_flag=None):
    tempGraph = nx.Graph()
    ...
    for i, (tax, val) in enumerate(series.items()):
        ...
        for j in range(0, len(bac[i].lst) - 1):
            updateval(tempGraph, bac[i], valdict, j, True, remove_flag)
        # adding the value of the last node in the chain
        updateval(tempGraph, bac[i], valdict, len(bac[i].lst) - 1, False, remove_flag)
    valdict["anaerobe"] = valdict[("Bacteria",)] + valdict[("Archaea",)]
    return create_final_graph(tempGraph, valdict)


def updateval(graph, bac, vald, num, adde, flag):
    if adde:
...
def create_final_graph(tempGraph, valdict):
    graph = nx.Graph()
