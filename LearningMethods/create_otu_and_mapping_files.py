...
from Plot.old.plot_rho import draw_X_y_rhos_calculation_figure
...
class CreateOtuAndMappingFiles(object):
    ...
    def preprocess(self, preprocess_params, visualize, ip):
        # print('preprocess...')
        ...
        if self.tags_path:
            self.otu_features_df, self.otu_features_df_b_pca, self.pca_ocj, self.bacteria, self.pca_comp = preprocess_data(
                self.otu_features_df, preprocess_params, self.tags_df, visualize_data=visualize, ip=ip)
        else:
            self.otu_features_df, self.otu_features_df_b_pca, self.pca_ocj, self.bacteria, self.pca_comp = preprocess_data(
                self.otu_features_df, preprocess_params, map_file=None, visualize_data=visualize, ip=ip)
