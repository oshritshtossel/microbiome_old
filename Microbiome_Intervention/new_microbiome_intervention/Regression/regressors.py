...
def get_model(model_name, model_params={}):
    if model_name == 'Linear_regression':
        ...
        if model_params=={}:
            return Lasso(alpha=0.1),1
        else:
            return Lasso(alpha=model_params['alpha']),1
    elif model_name=='Ridge':
        if model_params == {}:
            return Ridge(alpha=1.0),1
        else:
            return Ridge(alpha=model_params['alpha']),1
    elif model_name=='ARD':
        if model_params == {}:
            return ARDRegression(),1
        else:
            return ARDRegression(alpha_1=model_params['alpha'], lambda_1=model_params['lambda']),1
    elif model_name == 'SVR':
        if model_params == {}:
            return svm.SVR(kernel='linear', gamma='auto'),1
        else:
            return svm.SVR(C=model_params['c'], epsilon=model_params['epsilon'], kernel='linear', gamma='auto'),1
    elif model_name == 'SVR poly':
        return svm.SVR(kernel='poly', gamma='auto'),0
    elif model_name == 'SVR rbf':
        return svm.SVR(kernel='rbf', gamma='auto'),0
    elif model_name == 'SVR sigmoid':
        return svm.SVR(kernel='sigmoid', gamma='auto'),0
    elif model_name == 'SVR precomputed':
        return svm.SVR(kernel='precomputed', gamma='auto'),0
    else:
        print('model not supported'),0
        raise -1
