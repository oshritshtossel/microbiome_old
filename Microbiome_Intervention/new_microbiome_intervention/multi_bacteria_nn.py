...
import pickle
def run_multi_bacteria(xs_path, ys_path, name):
    ...
    df_xs = df_xs.iloc[:,1:]
    df_ys = df_ys.iloc[:,1:]
    df_ys = df_ys.loc[:, df_ys.nunique() >= 4]
    results = {}
    ...
    for STRUCTURE in structures_short:
        for LEARNING_RATE in [1e-3]:
            for REGULARIZATION in reg_long:
                for DROPOUT in dropout_long:
                    epochs = 70
                    ...
                    res_map = run_NN(df_xs, df_ys, missing_values=None, params=params, name='multi_nn_'+str(name), folder='../../../PycharmProjects/data_microbiome_in_time/' + name,
    ...
    with open('m_nn_' + name + '.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        run_multi_bacteria(datasets[dataset][0], datasets[dataset][1], datasets[dataset][2])